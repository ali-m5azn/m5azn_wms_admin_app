...
Future generateSellerPDF() async {
...
Future<String> getFilePath() async {
  ...
  return '${directory.path}/seller_invoice.pdf';
}

pw.Widget buildTableSale() {
  return pw.Column(children: [
    _buildTableHeader(),
    ...totalSellerList.map((e) => _buildTableRow(e)),
  ]);
}

pw.Widget _buildTableHeader() {
  return pw.Row(
    children: sellerLabel.asMap().entries.map((entry) {
...
pw.Widget _buildTableRow(TotalSellerModel item) {
  return pw.Row(
    children: [
      ...[item.shopName, item.lastName, item.username, item.email, item.phone]
          .asMap()
          .entries
          .map((entry) {
        final int index = entry.key;
